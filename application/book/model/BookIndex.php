...
class BookIndex extends Model {
    ...
    // );
    public function edits($isUpdate){
       $this->isUpdate($isUpdate)->allowField(true)->save($_POST);
       return $this->id;
    }
    public function lists(){
        $datas = $this->all(function($query){
            $query->where('status', '>', -1)->order('sort desc , id asc')->column('id,title,sort,status,pid');
        });
        if($datas){
    ...
    public function finds($id){
        $datas = $this->get(function($query)use($id){
            $query->where(['status' =>['>', -1],'id'=>['=',$id]]);
        });
        $data = $datas->toArray();
        unset($datas);
        return $data;
