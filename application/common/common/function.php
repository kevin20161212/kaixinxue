...
function returnUrlPath($v,$k){
  $url_path="";
  $url_arr=array();
  $pathflag = false; //判断地址参数是否已存在
  if(array_key_exists('page', $_GET)){
    unset($_GET['page']);
  }
  foreach ($_GET as $key => $value) {
    if($key != $k){
        if(!(empty($v) && $k=='brand' && $key=='car_brand')){
            $url_arr[]=$key."=".$value;
        }
    }
    else{
 ...
 * 拍卖列表判断样式自定义函数
 */
function returnActive($k,$id){
    $v = array_key_exists($k, $_GET) ? $_GET[$k] : '';
    if($v == $id){
        return "xuanzhogn";
    }else{
        return '';
