...
class Area extends Admin {
    ...
    public function index() {
        $model_object = D("HomeArea");
        // 获取列表
        $map = array();
        $keyword         = I('keyword','', 'string');
        $condition       = array('like', '%' . $keyword . '%');
        $map['id|name'] = array($condition);
        ...
        $attr['href']  = U('Area/del', array('id' => '__data_id__','model'=>'Home_District'));

        // 使用Builder快速建立列表页面
        $builder = new \yfthink\builder\ListBuilder();
        $builder->setMetaTitle("列表")  // 设置页面标题
                ->addTopButton("addnew",array('model'=>'Home_Area'))    // 添加新增按钮
                ->addTopButton("resume",array('model'=>'Home_Area'))  // 添加启用按钮
                ...
                ->addRightButton("forbid",array('model'=>'Home_Area'))  // 添加禁用/启用按钮
    ...
    public function add() {
        if (request()->isPost()) {
            ...
            $builder->setMetaTitle("新增")  // 设置页面标题
                    ...
                    ->addFormItem("name", "text", "区域名称", "例如：华北")
                    ->display();
        }
    }
