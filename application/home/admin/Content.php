...
class Content extends Admin {
    ...
    public function index() {
        ...
        $map['id|title'] = array($condition);
        //$map["status"] = array("egt", "0");  // 禁用和正常状态
        $p = $_GET["p"] ? : 1;
        $model_object = D("HomeContent");
        ...
        $attr2['href']  = U('Content/detail', array('id' => '__data_id__'));
        $attr['name']  = '删除';
        $attr['title'] = '删除';
        $attr['class'] = 'ajax-get label confirm label-danger-outline label-pill';
        $attr['href']  = U('Content/del', array('id' => '__data_id__','model'=>'Home_Content'));

        // 使用Builder快速建立列表页面
        $builder = new \yfthink\builder\ListBuilder();
        $builder->setMetaTitle("列表")  // 设置页面标题
                //->addTopButton("addnew")    // 添加新增按钮
                ->addTopButton("resume",array('model'=>'Home_Content'))  // 添加启用按钮
                ->addTopButton("forbid",array('model'=>'Home_Content'))  // 添加禁用按钮
                ->setSearch("请输入ID/标题", U("index"))
                ->addTableColumn("id", "ID")
                ->addTableColumn("title", "标题",'callback', array(D('HomeContent'),'GetPartStr'))
                ->addTableColumn("areaid", "城市",'callback', array(D('HomeDistrict'), 'get_name'))
                //->addTableColumn("url", "链接")
                ->addRightButton("self",$attr2)
                ->addTableColumn("time", "创建时间",'time')
                ->addTableColumn("status", "状态", "status")
                ->addTableColumn("right_button", "操作", "btn")
                ->setTableDataList($data_list)     // 数据列表
                ->setTableDataPage($page->show())  // 数据列表分页
                // ->addRightButton("edit")           // 添加编辑按钮
                ->addRightButton("forbid",array('model'=>'Home_Content'))  // 添加禁用/启用按钮
    ...
    public function add() {
        if (request()->isPost()) {
            ...
            if ($data) {
                $num = $model_object->where(['name'=>$data['name'],'dpid'=>$data['dpid']])->find();
                if($num){
                    $this->error('该信息已存在');
                }
            ...
            $builder->setMetaTitle("新增")  // 设置页面标题
                    ...
                    ->addFormItem('apid', 'select', '省级名称', '0', $data)
                    ->addFormItem("name", "text", "市区名称", "例如：济南")
    ...
    public function edit($id) {
        if (request()->isPost()) {
            ...
            $builder->setMetaTitle("编辑")  // 设置页面标题
                    ...
                    ->addFormItem("name", "text", "市区名称", "例如：济南")
                    ->addFormItem("intial", "text", "首字母", "例如：S")
                    ->setFormData(D("HomeCity")->find($id))
                    ->display();
        }
    }
