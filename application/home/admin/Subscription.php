...
class Subscription extends Admin {
    ...
    public function index() {
        ...
        $map['id|content'] = array($condition);
        ...
        $attr['href']  = U('Subscription/del', array('id' => '__data_id__','model'=>'HomeSubscriptionGroup'));
        // 使用Builder快速建立列表页面
        $builder = new \yfthink\builder\ListBuilder();
        $builder->setMetaTitle("列表")  // 设置页面标题
                // ->addTopButton("addnew",array())    // 添加新增按钮
                ->addTopButton("resume",array('model'=>'Home_Subscription_Group'))  // 添加启用按钮
                ->addTopButton("forbid",array('model'=>'Home_Subscription_Group'))  // 添加禁用按钮
                ...
                ->addRightButton("edit") 
                ->addRightButton("forbid",array('model'=>'Home_Subscription_Group'))  // 添加禁用/启用按钮          // 添加编辑按钮
    ...
    public function edit($id) {
     ...
     public function del($id){
        $map['id'] = $id;
        $re = D('HomeSubscriptionGroup')->del($map);
        if($re == 1){
