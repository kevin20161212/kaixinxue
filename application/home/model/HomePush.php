...
class HomePush extends Model {
     ...
     * @author youfai.cn <280962430@qq.com>
     */
    protected $tableName = 'Home_Push_Record';
    ...
    */
    ...
    public function lists($uid,$num=20,$page=1){
        $data = db("home_push_record")->alias('v')->join('__HOME_CONTENT__ c ','v.content_id= c.id')->where(['v.uid'=>$uid])->order('v.id desc')->limit($num)->page($page)->field('c.id,c.title,c.areaid,c.time,c.create_time')->select();
        $num = db("home_push_record")->alias('v')->join('__HOME_CONTENT__ c ','v.content_id= c.id')->where(['v.uid'=>$uid])->count();
        //$data=$data->toArray();
