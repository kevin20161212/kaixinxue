      ...
      formObj.children("input").each(function(){
       ...
       if(!proContinue){return false} //被检测信息有不合法情况，直接退出
    	...
    	else
    	{
    		...
    		if($(that).hasClass("confirm")){gl.layer_dialog_2(succ,"确定要提交信息吗?",action,query,false)}else{layer.load();gl.sendAjax(action,query,succ)}
    	}
    })
    /*
     * 移动端头部导航
...
GLJS.prototype.layer_dialog_2=function(succ,message,httpUrl,params,isFd){
		if(httpUrl)
		{
			...
			{
				if(isFd){layer.load();gl.sendFormData(httpUrl,params,succ)}else{layer.load();gl.sendAjax(httpUrl,params,succ)}
			});
		}
...
GLJS.prototype.sendAjax=function(httpUrl,params,callBack){
		...
		$.ajax({
			...
			error:function(){layer.closeAll('loading');layer.alert("网络异常",{icon:5})}
		})
},
