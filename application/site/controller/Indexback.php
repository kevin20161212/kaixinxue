    ...
    public function index(){
		...
		$configs = array(
			...
			'list_url_regexes' => array(
				'http://www.zycg.cn/home/jqkbxm\?catalog=StockAffiche&page=\d+'
			),
		...
		$spider->on_extract_page = function($page, $data) 
		{
		    $data = preg_replace("#<!--.*-->#" , '' , $data); 
     		$data = preg_replace("#<!－－.*－－>#" , '' , $data); 
     		$data['url'] = $page['request']['url'];
     		self::set_cachedata($data);
     		dump($page);
		    return false;
     ...
     public static function set_cachedata($data){
     	$keydate = date('Ymd').$data['article_title'];
     	$cachedata = cache('keycache');
     	// if(!cache($keydate)){
     		Cache::set($keydate,$data);
	     	$datas = cache('keycache');
		    $num = $datas?count($datas):0;
		    $datas[$num] = $keydate;
		    cache('keycache',$datas);
		    unset($datas);
		    return true;
     	// }else{
     	// 	return false;
     ...
     public function indexs(){
       ...
       //  	}
     	...
     	// dump(count($data));
     	exit;
     	Cache::tag('tags')->set('你好','v123456');
		Cache::tag('tag')->set('name2','value2');
		// $data = 
		dump(Cache::tag('tags')->get('你好'));
