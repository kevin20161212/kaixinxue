...
class Category extends Admin {
    ...
    public function index() {
        $data_list=D('shop_category')->treelists();
        // 使用Builder快速建立列表页面
        ...
        $builder->setMetaTitle("列表")  // 设置页面标题
                ...
                ->addRightButton("forbid",array('model'=>'shop_category'))  // 添加禁用/启用按钮
                ->addRightButton("delete",array('model'=>'shop_category'))  // 添加删除按钮
                ->display();
    }
    ...
    public function add() {
        if (request()->isPost()) {
            $data = D("shop_category")->editcat(false);
    ...
    public function edit($id) {
        if (request()->isPost()) {
            $data = D("shop_category")->editcat(true);
            if ($data) {
                $this->success("新增成功", U("index"));
            } else {
                $this->error("新增失败");
