...
class Push
{
    ...
    public function notify()
    {
        ...
        // $token =WechatService::getAccessToken();
         // file_put_contents('/alidata/www/spider.youfai.cn/application/token222.txt',$this->getAccessToken());
        ...
        switch ($info['type']) {
            ...
            case 'news':
                ...
                foreach ($news['articles'] as $vo) {
                    ...
                    $data[] = ['Title'=>$vo['title'],'Description'=>$vo['digest'],'PicUrl' =>'http://lixiakonggu.youfai.cn'.$vo['local_url'],'Url' =>$url];
                    //$data[] = ['Url' => $url, 'Title' => $vo['title'], 'Picurl' =>'http://spider.youfai.cn'.$vo['local_url'], 'Description' => $vo['digest']];
                }
               file_put_contents('/alidata/www/spider.youfai.cn/application/tt.txt', var_export($data,true));
    ...
    protected function sendMessage($type, $data)
    {
        ...
        $msgData =array('touser'=> $this->openid,'msgtype'=>$type,'"{$type}'=>$data);
        ...
        switch (strtolower(sysconf('wechat_type'))) {
            case 'api': // 参数对接，直接回复XML来实现消息回复
                ...
                switch (strtolower($type)) {
                    case 'text':
                      file_put_contents('/alidata/www/spider.youfai.cn/application/text2222.txt',$wechat->text($data['content']));
                    ...
                    case 'news':
                      file_put_contents('/alidata/www/spider.youfai.cn/application/text2222.txt',$wechat->news($data['articles'])->reply([], true));
                      // file_put_contents('/alidata/www/spider.youfai.cn/application/text22.txt', $wechat->image($data['media_id'])->reply([], true));
