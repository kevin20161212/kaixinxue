...
class WechatService
{
    ...
    {
        $signUrl = is_null($url) ?request()->url(true) : $url;
        switch (strtolower(sysconf('wechat_type'))) {
            case 'api':
    ...
    public static function getWxConfig()
    {
        ...
        switch (strtolower(sysconf('wechat_type'))) {
            case 'api':
                $config['wechat_appid'] = sysconf('wechat_appid');
                $config['wechat_appsecret'] = sysconf('wechat_appsecret');
                $wechat = WechatService::WeChatOauth();
                $config['access_token']  = $wechat->getAccessToken();
                return $config;
            case 'thr':
                return sysconf('wechat_thr_appsecre');
            default:
                throw new Exception('请在后台配置微信对接授权模式！');
        }
    }
