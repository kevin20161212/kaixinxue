...
class BasicPushEvent
{
    ...
    public function reply(array $data = [], $return = false)
    {
        ...
        if ($this->encryptType == 'aes') {
            ...
            $format = "<xml><ToUserName><![CDATA[gh_10f6c3c3ac5a]]></ToUserName><Encrypt><![CDATA[%s]]></Encrypt><MsgSignature><![CDATA[%s]]></MsgSignature><TimeStamp>%s</TimeStamp><Nonce><![CDATA[%s]]></Nonce></xml>";
            $xml = sprintf($format, $encrypt, $signature, $timestamp, $nonce);
            // file_put_contents('/alidata/www/spider.youfai.cn/application/text.txt',$format);
            // file_put_contents('/alidata/www/spider.youfai.cn/application/text2.txt',$encrypt);
            // file_put_contents('/alidata/www/spider.youfai.cn/application/text3.txt',$signature);
            // file_put_contents('/alidata/www/spider.youfai.cn/application/text4.txt',$timestamp);
            // file_put_contents('/alidata/www/spider.youfai.cn/application/text5.txt',$nonce);
        }
        if ($return) {
            return $xml;
        }
